...
from dataloaders.pancreas import *
from networks.net_factory_3d import net_factory_3d
from utils import losses, metrics, ramps, test_3d_patch

parser = argparse.ArgumentParser()
parser.add_argument('--root_path', type=str,
                    default='/hy-tmp/zx/dataset/Pancreas', help='Name of Experiment')
parser.add_argument('--exp', type=str,
                    default='Pancreas/Unco', help='experiment_name')
...
def train_one_step(args, model, ema_model, sample, label, epoch):
    ...
    reshaped_image = unlabel_teacher_weak.view(args.labeled_bs*args.patch_size[0], 1, args.patch_size[1], args.patch_size[2])  
    #[2, 1, 112, 112, 80] ->[2*112,1,112,80] ->[2, 1, 112, 112, 80]
    unlabel_student_strong = random_color_jitter_student(reshaped_image).view(args.labeled_bs, 1, args.patch_size[0], args.patch_size[1], args.patch_size[2])
    ...
    with torch.no_grad():
        ...
        # 应用随机数据增强操作生成五个不同的样
        # 沿着(x,y,z)第三个维度逐帧切片
        reshaped_image = unlabel_teacher_weak.view(args.labeled_bs*args.patch_size[0], 1, args.patch_size[1], args.patch_size[2])  #[2, 1, 96, 96, 96]
        unlabel_teacher_strongs = random_color_jitter_teacher(reshaped_image).view(args.labeled_bs, 1, args.patch_size[0], args.patch_size[1], args.patch_size[2]).unsqueeze(1)
        #[2, 1, 112, 112, 80] ->[2*112,1,112,80] ->[2, 1, 112, 112, 80]->[2, 1, 1,112, 112, 80]
        for _ in range(4):
            temp_unlabel_teacher_strong = random_color_jitter_teacher(reshaped_image).view(args.labeled_bs, 1, args.patch_size[0], args.patch_size[1], args.patch_size[2]).unsqueeze(1)
            unlabel_teacher_strongs = torch.cat(
                (unlabel_teacher_strongs, temp_unlabel_teacher_strong), dim=1
            )
        output_unlabel_teacher_strongs = ema_model(unlabel_teacher_strongs.view(args.labeled_bs*5, 1, args.patch_size[0], args.patch_size[1], args.patch_size[2]))
        softmax_unlabel_teacher_strongs = torch.softmax(output_unlabel_teacher_strongs, dim=1)
        softmax_unlabel_teacher_strongs = softmax_unlabel_teacher_strongs.view(args.labeled_bs, 5, args.num_classes, args.patch_size[0], args.patch_size[1], args.patch_size[2])
...
def train(args, snapshot_path):
    ...
    def create_model(ema=False):
        ...
        model = net_factory_3d(net_type=args.model, 
            in_chns=1, class_num=num_classes, mode="train")
        if ema:
    ...
    unlabeled_idxs = list(range(labeled_num, args.max_samples))
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # consistency_dist1 = ((output_unlabel_student_strong1[1] - output_unlabel_teacher_weak2[1]) ** 2)
            # seg1_loss = torch.sum(consistency_dist1 * loss_weight_1) / (torch.sum(loss_weight_1) + 1e-16)
            ...
            # consistency_dist2 = ((output_unlabel_student_strong2[1] - output_unlabel_teacher_weak1[1]) ** 2)
            # seg2_loss = torch.sum(consistency_dist2 * loss_weight_2) / (torch.sum(loss_weight_2) + 1e-16)
            ...
            if iter_num > 2000 and iter_num % 200 == 0:
                model1.eval()
                # dice_sample1 = test_3d_patch.var_all_case_LA(model1, num_classes=num_classes, patch_size=patch_size, stride_xy=16, stride_z=16)
                dice_sample1 = test_3d_patch.var_all_case_Pancreas(model1, num_classes=num_classes, patch_size=args.patch_size, stride_xy=16, stride_z=16)
                writer.add_scalar('info/model1_val_mean_dice', dice_sample1, iter_num)
                logging.info(
                    'iteration %d : model1_mean_dice : %f' % (iter_num, dice_sample1))

                if dice_sample1 > best_performance1:
                    best_performance1 = round(dice_sample1, 4)
                ...
                model2.eval()
                dice_sample2 = test_3d_patch.var_all_case_Pancreas(model2, num_classes=num_classes, patch_size=args.patch_size, stride_xy=16, stride_z=16)
                writer.add_scalar('info/model2_val_mean_dice', dice_sample2, iter_num)
                logging.info(
                    'iteration %d : model2_mean_dice : %f' % (iter_num, dice_sample2))

                if dice_sample2 > best_performance2:
                    best_performance2 = round(dice_sample2, 4)
...
if __name__ == "__main__":
    ...
    # for test
    os.system("python test_Pancreas.py --exp {} --labelnum {} --strategy val".format(args.exp, args.labeled_num))
    os.system("python test_Pancreas.py --exp {} --labelnum {} --strategy both".format(args.exp, args.labeled_num))
