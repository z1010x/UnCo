...
parser.add_argument('--exp', type=str,
                    default='Pancreas/Unco_2', help='experiment_name')
...
with open('/hy-tmp/zx/dataset/Pancreas/test.list', 'r') as f:
            image_list = f.readlines()
image_list = ["/hy-tmp/zx/dataset/Pancreas/Pancreas_h5/" + item.replace('\n', '') + ".h5" for item in image_list]
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                with torch.no_grad():
                    ...
                    confidence = torch.max(avg_pred, dim=1)
                    # Get the prediction with higher confidence
                    y = torch.argmax(avg_pred, dim=1)
...
def test_all_case_both(model, model1, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=False, test_save_path=None, metric_detail=0):
